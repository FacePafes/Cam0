...
If it's something you'd rather not post publicly (security related/adjacent stuff), email me via the email on my github profile.

## Device compatibility

If you build it and it works (or doesn't) on a device that's not
already listed in the [README](README.md), saying so is helpful, even
informally a comment on an issue like "built and ran fine on a
[device]" counts. Nothing formal is setup at the current MVP stage, no official
device support list being tracked yet as that is very much a post MVP problem.

## If you're looking at the code anyway
...
- No new dependencies without discussing it first. GuiLite for the UI, CameraX for the camera, otherwise plain Android framework
...
- If it touches `core/camera_overlay.cpp`, it should still compile
  ...
  before it ever needs the Android build.
- `third_party/GuiLite/GuiLite.h` has one intentional local patch
  ...
  binary, survives `strip`, ships in release builds.
